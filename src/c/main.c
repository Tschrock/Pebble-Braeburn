...
#define seperate_time

static void initVars() {
  timePos = GRect(27, 1, 80, 70);
  timePos2 = GRect(93, 25, 54, 45);
  bgPos = GRect(0, 0, 144, 168);
  batteryPos = GRect(119, 7, 17, 10);
  batteryIconSize = GRect(0, 0, 17, 10);
  bluetoothPos = GRect(7, 6, 13, 12);
  bluetoothIconSize = GRect(0, 0, 13, 12);
  datePos = GRect(0, 143, 144, 25);
}
...
static void main_window_load(Window *window) {
  ...
  ResHandle font_handle = resource_get_handle(RESOURCE_ID_ROBOTO_BOLD_60);
  ResHandle font_handle3 = resource_get_handle(RESOURCE_ID_ROBOTO_BOLD_36);
  ...
  layer_add_child(window_get_root_layer(window), text_layer_get_layer(s_time_layer));
    ...
    // Improve the layout to be more like a watchface
    text_layer_set_font(s_time_layer2, fonts_load_custom_font(font_handle3));
